...
if len(stress_corr) > 0:
    ...
    with chart_card:
        ...
        if len(etf_prices) > 0:
            ...
            fig_stress.add_trace(go.Scatter(
                x=[price_df['Date'].min(), price_df['Date'].max()],
            ...
            st.markdown("<small>*Colored regions show top 10 drawdown periods. Red line connects mean correlation at midpoint of each drawdown. Blue dashed line = normal correlation baseline.*</small>", unsafe_allow_html=True)
        else:
